    ...
    if success:
        if Config.USE_ENV_CONFIG:
            return jsonify(
                {
                    "success": True,
                    "message": "Configuration updated in memory. Using environment variable - changes will be lost on restart. Update REGISTRIES env var to persist.",
                }
            )
        else:
            return jsonify(
...
def api_create_registry():
    ...
    if Config.save_registries():
        ...
        return jsonify(
            {
                "success": True,
                "message": "Registry created (in-memory only - using env config)",
            }
        )
            ...
            for tag in tags:
                ...
                if not dry_run:
                    try:
                        ...
                        if result.get("error"):
                            logger.error(
                                f"Scan failed for {repo}:{tag}: {result.get('error')}"
                            )
