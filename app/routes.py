...
@api_bp.route(
    "/vulnerabilities/<registry_name>/<path:repo>/<tag>/download", methods=["GET"]
)
def api_download_vuln_json(registry_name, repo, tag):
    ...
    if not result:
        # Fallback: try persisted per-image file
        data_dir = (
        ...
        try:
            if os.path.exists(image_file):
                ...
                if file_result.get("repo") == repo and file_result.get("tag") == tag:
                    result = file_result
        except Exception as e:
            logger.warning(f"Failed fallback load for {repo}:{tag}: {e}")
...
@main_bp.route("/docs/")
def docs_index():
    return send_from_directory(_docs_directory(), "index.html")


@main_bp.route("/docs/<path:doc_path>")
def docs_static(doc_path):
    return send_from_directory(_docs_directory(), doc_path)
...
def api_bulk_operation():
    ...
    for repo in repos:
        ...
        if tags_to_delete:
            results.append(
                {"repo": repo, "tags": tags_to_delete, "count": len(tags_to_delete)}
            )
...
def api_update_registry_config():
    ...
    if success:
        return jsonify(
            {
                "success": True,
                "message": "Configuration saved to file and persisted.",
            }
        )
    else:
        return jsonify({"success": False, "error": "Failed to update registry"}), 500
...
def api_create_registry():
    ...
    if Config.save_registries():
        return jsonify(
            {"success": True, "message": "Registry created and saved to file"}
        )
    else:
        return jsonify({"success": False, "error": "Failed to persist registry"}), 500
...
def api_scan_all(registry_name):
    ...
    if not vuln_scan.get("enabled"):
        return jsonify(
            {"success": False, "error": "Vulnerability scanning not enabled"}
        ), 400
    ...
    try:
        ...
        scanner = get_scanner(
            vuln_scan.get("scanner", "trivy"), vuln_scan.get("scannerUrl")
        )
        auth = get_auth(registry)
        repos, error = fetch_repositories(registry["api"], auth)
...
def api_scan_massive(registry_name):
    ...
    try:
        ...
        if data.get("sendRecap", False):
            send_massive_scan_recap(registry_name, result)
...
def api_system_status():
    """Get scheduler and notification runtime status"""
    ...
    return jsonify(
        {
            ...
            "notifications": {
                "emailEnabled": Config.NOTIFY_EMAIL_ENABLED,
                "telegramEnabled": Config.NOTIFY_TELEGRAM_ENABLED,
            },
            "storage": {
