...
function loadTags(registryName, repo) {
  ...
  if (deleteBtn) {
    deleteBtn.style.display =
      typeof readOnly !== "undefined" && readOnly ? "none" : "block";
  }
  ...
  fetch(
    `/api/tags/${encodeURIComponent(registryName)}/${encodeURIComponent(repo)}`,
  )
    .then((r) => r.json())
    .then((data) => {
      if (data.error) {
      ...
      const tagControls = document.getElementById("tag-controls");
      if (tagControls)
        tagControls.style.display = tags.length > 0 ? "flex" : "none";
            ...
            <div class="mb-2">
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <div class="btn-group btn-group-sm tag-btn-group" data-tag="${tag}">
                        ...
                        ${
                          !readOnly
                            ? `<button class="btn btn-outline-danger" onclick="deleteTag('${currentRegistry}', '${currentRepo}', '${tag}')">
...
function loadTagSizes(registryName, repo, tags) {
  ...
  function loadBatch() {
    ...
    batch.forEach((tag) => {
      fetch(
        `/api/tag-details/${encodeURIComponent(registryName)}/${encodeURIComponent(repo)}/${encodeURIComponent(tag)}`,
      )
        .then((r) => r.json())
        .then((data) => {
          ...
          const createdEl = document.querySelector(
            `.tag-created[data-tag="${tag}"]`,
          );
          ...
          const digestEl = document.querySelector(
            `.tag-digest[data-tag="${tag}"]`,
          );
  ...
  tagItems.sort((a, b) => {
    ...
    if (sortBy === "name") {
      ...
      const sizeA = parseInt(
        a.querySelector(".tag-size").dataset.sizeBytes || 0,
      );
      const sizeB = parseInt(
        b.querySelector(".tag-size").dataset.sizeBytes || 0,
      );
      return sizeB - sizeA;
    } else if (sortBy === "date") {
      const dateA = parseInt(
        a.querySelector(".tag-created").dataset.timestamp || 0,
      );
      const dateB = parseInt(
        b.querySelector(".tag-created").dataset.timestamp || 0,
      );
      return dateB - dateA;
    }
    return 0;
    ...
    .then((data) => {
      ...
      tags.forEach((tag) => {
        ...
        const btnEl = document.querySelector(
          `.tag-scan-btn[data-tag="${tag}"]`,
        );
        const downloadBtn = document.querySelector(
          `.tag-download-json-btn[data-tag="${tag}"]`,
        );
        const cveBtn = document.querySelector(
          `.tag-cve-btn[data-tag="${tag}"]`,
        );
        let total = 0;
        if (result) {
        ...
        if (downloadBtn) {
          if (total > 0) {
            ...
            downloadBtn.onclick = function () {
              window.open(
                `/api/vulnerabilities/${encodeURIComponent(currentRegistry)}/${encodeURIComponent(currentRepo)}/${encodeURIComponent(tag)}/download`,
                "_blank",
              );
            };
          } else {
        ...
        if (result && vulnEl && btnEl) {
          ...
          let badges =
            '<div><small class="text-muted">Vulnerabilities:</small></div>';
...
function scanTagVulnerabilities(registryName, repo, tag, btn) {
  ...
  fetch(
    `/api/scan/${encodeURIComponent(registryName)}/${encodeURIComponent(repo)}/${encodeURIComponent(tag)}`,
  )
    .then((r) => r.json())
    .then((data) => {
      btn.disabled = false;
      ...
      if (data.error) {
        ...
        if (total === 0) {
          ...
          showAlert(
            `Found ${total} vulnerabilities (Critical: ${critical}, High: ${high}) in ${repo}:${tag}`,
            "warning",
          );
        }
        ...
        document
          ...
          .addEventListener("change", function () {
            const rows = document.querySelectorAll(
              `#cve-table tbody tr[data-severity="${severity.toUpperCase()}"]`,
            );
            rows.forEach((row) => {
...
function deleteTag(registryName, repo, tag) {
  ...
  pendingDelete = function () {
    fetch(
      `/api/delete/tag/${encodeURIComponent(registryName)}/${encodeURIComponent(repo)}/${encodeURIComponent(tag)}`,
      { method: "DELETE" },
    )
      .then((r) => r.json())
      .then((data) => {
        if (data.success) {
          showAlert(
            `Tag <strong>${tag}</strong> deleted successfully. <a href="#" onclick="switchToRegistryConfig(); return false;" class="alert-link">Run garbage collection</a> to free disk space.`,
            "warning",
          );
