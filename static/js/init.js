...
window.addEventListener("DOMContentLoaded", function () {
  ...
  document.querySelectorAll("[data-view]").forEach((link) => {
    link.addEventListener("click", function (e) {
      ...
      document.getElementById("repositories-view").style.display =
        view === "repositories" ? "block" : "none";
      document.getElementById("registries-view").style.display =
        view === "registries" ? "block" : "none";
      document.getElementById("cleanup-view").style.display =
        view === "cleanup" ? "block" : "none";
      document.getElementById("bulk-operations-view").style.display =
        view === "bulk" ? "block" : "none";
      document.getElementById("massive-scan-view").style.display =
        view === "massivescan" ? "block" : "none";
      document.getElementById("system-status-view").style.display =
        view === "status" ? "block" : "none";
  ...
  if (deleteRepoBtn) {
    deleteRepoBtn.addEventListener("click", function () {
      ...
      pendingDelete = function () {
        fetch(
          `/api/delete/repo/${encodeURIComponent(currentRegistry)}/${encodeURIComponent(currentRepo)}`,
          { method: "DELETE" },
        )
          .then((r) => r.json())
          .then((data) => {
            if (data.success) {
              showAlert(
                `Repository <strong>${currentRepo}</strong> deleted (${data.deleted}/${data.total} tags). <a href="#" onclick="switchToRegistryConfig(); return false;" class="alert-link">Run garbage collection</a> to free disk space.`,
                "warning",
              );
              loadRepositories(currentRegistry, true);
              document.getElementById("image-name-section").style.display =
                "none";
  ...
  if (bulkOpBtn) {
    bulkOpBtn.addEventListener("click", function () {
      ...
      const olderThan =
        parseInt(document.getElementById("bulkOlderThan").value) || null;
      const keepMin =
        parseInt(document.getElementById("bulkKeepMin").value) || 0;

      if (olderThan && keepMin < 1) {
        showAlert(
          'ERROR: "Keep minimum tags" is REQUIRED when using age-based deletion to prevent losing all images. Set it to at least 5.',
          "danger",
        );
      ...
      document.getElementById("bulkResults").innerHTML =
        '<div class="spinner-border"></div> Processing...';
  ...
  fetch("/api/registries")
    ...
    .then((data) => {
      data.registries.forEach((reg) => {
        registryUrlCache[reg.name] =
          reg.url || reg.api.replace(/^https?:\/\//, "");
      });
    });

  const selector = document.getElementById("registrySelector");
  if (selector) {
    const defaultOption = Array.from(selector.options).find(
      (opt) => opt.dataset.default === "true",
    );
    const firstOption = Array.from(selector.options).find(
      (opt) => opt.value !== "",
    );
  ...
  const downloadAllConfigBtn = document.getElementById(
    "download-all-config-btn",
  );
  ...
  if (cancelAddBtn) {
    cancelAddBtn.addEventListener("click", function () {
      ...
      document.getElementById("add-registry-btn").style.display =
        "inline-block";
  ...
  if (inlineAuth) {
    inlineAuth.addEventListener("change", function () {
      document.getElementById("inline-auth-fields").style.display = this.checked
        ? "block"
        : "none";
    });
  }
    ...
    inlineForm.addEventListener("submit", function (e) {
      ...
      fetch("/api/registry/create", {
        ...
        .then((data) => {
          if (data.success) {
            showAlert(
              `Registry "${registry.name}" added successfully! <button class="btn btn-sm btn-success ms-2" onclick="downloadConfig()"><i class="bi bi-download"></i> Download Backup</button> Refresh page to see changes.`,
              "success",
            );
            document.getElementById("add-registry-form").style.display = "none";
            document.getElementById("add-registry-btn").style.display =
