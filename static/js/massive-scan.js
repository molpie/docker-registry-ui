...
function updateMassiveScanSummary() {
  const registry =
    currentMassiveScanRegistry ||
    (document.getElementById("massiveScanRegistry")
      ? document.getElementById("massiveScanRegistry").value
      : null);
  const repoPattern = document.getElementById("massiveScanRepoPattern")
    ? document.getElementById("massiveScanRepoPattern").value
    : "*";
  ...
  const ageValue = document.getElementById("massiveScanAgeValue")
    ? document.getElementById("massiveScanAgeValue").value
    : 30;
  const ageUnit = document.getElementById("massiveScanAgeUnit")
    ? document.getElementById("massiveScanAgeUnit").value
    : "days";
  const includeAllTags = document.getElementById("massiveScanIncludeAllTags")
    ? document.getElementById("massiveScanIncludeAllTags").checked
    : true;
  const dryRun = document.getElementById("massiveScanDryRun")
    ? document.getElementById("massiveScanDryRun").checked
    : true;
  ...
  let html =
    '<div class="alert alert-info mb-0"><h6 class="alert-heading"><i class="bi bi-list-check"></i> Scan Configuration:</h6><ul class="mb-0">';
  ...
  if (!dryRun) {
    html +=
      '<div class="alert alert-warning mt-2 mb-0"><i class="bi bi-exclamation-triangle"></i> This will perform actual vulnerability scans which may take a long time!</div>';
  }
...
function populateMassiveScanRepoDropdown(registryName) {
  if (!registryName) {
    ...
    if (select) {
      select.innerHTML =
        '<option value="*">* (Nessun registry selezionato)</option>';
      select.disabled = true;
    }
    document.getElementById("massiveScanSummary").innerHTML =
      '<p class="text-muted">Seleziona un registry per iniziare</p>';
    return;
  }
...
function runMassiveScan() {
  ...
  const includeAllTags = document.getElementById(
    "massiveScanIncludeAllTags",
  ).checked;
  ...
  const timeoutId = setTimeout(() => {
    ...
    if (typeof showAlert === "function")
      showAlert("Scan timed out after 10 minutes.", "warning");
  }, 600000);
  ...
  fetch(`/api/scan-massive/${encodeURIComponent(registry)}`, {
    ...
    .then((result) => {
      ...
      if (!dryRun) {
        document.getElementById("massiveScanProgressBarInner").style.width =
          "100%";
        setTimeout(() => {
          document.getElementById("massiveScanProgressBar").style.display =
            "none";
          document.getElementById("massiveScanProgressBarInner").style.width =
            "0%";
        }, 500);
...
function renderMassiveScanResults() {
  ...
  if (massiveScanResults.length === 0) {
    container.innerHTML =
      '<p class="text-muted">Run scan to see results...</p>';
    return;
  }

  let html =
    '<div class="table-responsive"><table class="table table-sm table-hover"><thead><tr><th>Image</th><th>Status</th><th>Vulnerabilities</th></tr></thead><tbody>';
  ...
  if (massiveScanResults.length > 50) {
    html +=
      '<p class="text-muted text-center">Showing last 50 of ' +
      massiveScanResults.length +
      " results</p>";
  }
...
function initMassiveScan() {
  ...
  if (includeAllTags)
    includeAllTags.addEventListener("change", updateMassiveScanSummary);
  ...
  if (runBtn) {
    runBtn.addEventListener("click", function () {
      if (!document.getElementById("massiveScanDryRun").checked) {
        if (confirm("This will perform actual vulnerability scans. Continue?"))
          runMassiveScan();
      } else runMassiveScan();
    });
  }
